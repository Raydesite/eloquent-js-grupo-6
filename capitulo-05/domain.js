...
Escribe una función que calcule la dirección de
 escritura dominante en una cadena de texto. Recuerda que
 cada objeto de escritura tiene una propiedad de dirección que puede
 ser "ltr" (de izquierda a derecha), "rtl" (de derecha a izquierda),
 o "ttb" (de arriba a abajo).

La dirección dominante es la dirección de la mayoría de
los caracteres que tienen un script asociado.

Las funciones characterScript y countBy definidas
anteriormente en el capítulo son probablemente útiles aquí.
...
const text1 = 'Hello!' // ltr
const text2 = 'Hey, مساء الخير' // rtl
const text3 = '英国的狗说"woof' // ltr
const text4 = '俄罗斯的狗说"тяв"' // ltr
const text5 = 'Бүгдээрээ сайн байцгаана уу.'

/**
 * @param {string} text
 */
function detextDirection (text) {
  const direccion = { ltr: 0, rtl: 0, ttb: 0 }

  for (let i = 0; i < text.length; i++) {
    const charCode = text.charCodeAt(i)
    const idioma = SCRIPTS.filter((value) => {
      return value.ranges.filter(ranges => {
        return (charCode >= ranges[0] && charCode <= ranges[1])
      }).length > 0
    })

    if (idioma.length > 0) {
      direccion[idioma[0].direction]++
    }
  }

  const result = Object.entries(direccion) /*[ [key, value], ...] */
    .reduce((acc, value) => {
...
detextDirection(text1)
detextDirection(text2)
detextDirection(text3)
detextDirection(text4)
detextDirection(text5)
